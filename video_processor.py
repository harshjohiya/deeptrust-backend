...
class VideoProcessor:
    ...
    def sample_frames(self, video_path: str, num_frames: int = None) -> List[np.ndarray]:
    ...
    def extract_face(self, image: np.ndarray) -> np.ndarray:
        ...
        if not results.detections:
            # No face detected, return resized original image
            ...
            for idx, (frame, timestamp) in enumerate(zip(frames, timestamps)):
                ...
                if explainer:
                    try:
                        ...
                        print(f"Error generating heatmap for frame {idx}: {e}")
